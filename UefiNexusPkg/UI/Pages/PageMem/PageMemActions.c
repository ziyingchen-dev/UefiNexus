    ...
    )
{
    ...
    if (!PageCoreIsRangeValid(Address, PageState->CursorState.Width)) {
        Adapters->Error->ReportError(
    ...
    )
{
    ...
    if (!PageCoreIsRangeValid(PageState->CursorState.Address, PAGE_SIZE)) {
        ClearStatusLine(Adapters->Tui);
    ...
    )
{
    ...
    if (PrevAddress != PageState->CursorState.Address &&
        PageCoreIsRangeValid(PrevAddress, PAGE_SIZE)) {
    ...
    )
{
    ...
    if (NextAddress != PageState->CursorState.Address &&
        PageCoreIsRangeValid(NextAddress, PAGE_SIZE)) {
