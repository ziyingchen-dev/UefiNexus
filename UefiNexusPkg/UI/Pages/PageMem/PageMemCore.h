...
/**
  Update the cursor access width and realign the current offset to the new width.

  @param[in,out] State  Pointer to cursor state.
  @param[in]     Width  New access width in bytes.
**/
void
PageCoreSetCursorWidth(
    PAGEMEM_CURSOR_STATE *State,
    NX_UINTN              Width
    );
...
/**
  ...
  @param[out] Map              Pointer to address-map context to initialize.
  ...
  @retval NX_STATUS_INVALID_PARAMETER  Map input or descriptor input is invalid.
**/
NX_STATUS
...
/**
  ...
  @param[in] Map     Pointer to address-map context.
