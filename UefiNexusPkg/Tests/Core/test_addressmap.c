...
TestInitAndFirstAddress(void)
{
    ...
    ASSERT_EQ(PageCoreInitAddressMap(&Map, gDescriptors, 3), NX_STATUS_SUCCESS, "Init address map");
    ASSERT_EQ(PageCoreGetFirstValidAddress(&Map), 0x1000, "First valid address");
...
TestAddressValidation(void)
{
    ...
    PageCoreInitAddressMap(&Map, gDescriptors, 3);

    ASSERT_TRUE(PageCoreIsAddressValid(&Map, 0x1000) == NX_TRUE, "Valid conventional address");
    ASSERT_TRUE(PageCoreIsAddressValid(&Map, 0x1FFF) == NX_TRUE, "Valid address inside descriptor");
    ASSERT_TRUE(PageCoreIsAddressValid(&Map, 0x0500) == NX_FALSE, "Invalid reserved address");
...
TestRangeValidation(void)
{
    ...
    ASSERT_TRUE(PageCoreIsRangeValid(&Map, 0x1000, 0x100) == NX_TRUE, "Valid range");
    ASSERT_TRUE(PageCoreIsRangeValid(&Map, 0x2F80, 0x200) == NX_FALSE, "Range crossing boundary");
...
TestPageNavigation(void)
{
    ...
    ASSERT_EQ(PageCoreGetNextValidPageAddress(&Map, 0x1000, PAGE_SIZE), 0x1100, "Next valid page");
    ASSERT_EQ(PageCoreGetPreviousValidPageAddress(&Map, 0x1100, PAGE_SIZE), 0x1000, "Previous valid page");
    ASSERT_EQ(PageCoreGetNextValidPageAddress(&Map, 0x2F00, PAGE_SIZE), 0x4000, "Skip to next valid descriptor");
