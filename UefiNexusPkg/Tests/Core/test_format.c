...
//
// Validate truncation handling for undersized output buffers.
//
static
int
TestFormatValueLETruncation(void)
{
    NX_CHAR Buffer[4] = { 'X', 'X', 'X', '\0' };

    ASSERT_EQ(PageCoreFormatValueLE(0x1234, 2, Buffer, 3), 0, "Truncated buffer returns 0");
    ASSERT_TRUE(Buffer[0] == '\0', "Truncated buffer is cleared");
...
RunFormatTests(void)
{
    ...
    TEST(TestFormatValueLETruncation, "Format Value LE Truncation");
